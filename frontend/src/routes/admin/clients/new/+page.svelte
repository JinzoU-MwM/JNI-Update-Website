<script lang="ts">
  ...
  function handleFileSelect(e: Event) {
    ...
    if (input.files && input.files[0]) {
      ...
      form.logo_path = ''; // Clear URL when file is selected
    }
  }
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      if (!form.logo_path) {
        throw new Error('Please select a file or enter a logo URL');
      }

      const res = await adminPost('/clients', form);
...
<div class="page">
  ...
  <form onsubmit={handleSubmit} class="form">
    ...
    <div class="form-group">
      ...
      <div class="file-input-wrapper">
        ...
        <div class="file-input-display">
          ...
          {:else}
            <span class="file-placeholder">Click to select a logo (JPEG, PNG, GIF, WebP - max 5MB)</span>
          {/if}
        </div>
      </div>
    </div>
    ...
    <div class="form-group"><label>Logo URL</label><input type="url" bind:value={form.logo_path} placeholder="https://example.com/logo.png" disabled={!!selectedFile} /></div>
