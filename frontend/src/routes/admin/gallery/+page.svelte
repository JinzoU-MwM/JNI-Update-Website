<script lang="ts">
  ...
  async function deleteItem(id: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed');
      toasts.success('Foto berhasil dihapus');
      await load();
    } catch (err) {
      toasts.error(err instanceof Error ? err.message : 'Gagal menghapus foto');
    } finally {
      deleting = null;
    }
  }
      ...
      {#each gallery as item}
        <div class="card">
          <div class="img-wrap">
            ...
            <div class="overlay">
              <button onclick={() => deleteItem(item.id)} class="btn del" disabled={deleting === item.id}>
                {#if deleting === item.id}
